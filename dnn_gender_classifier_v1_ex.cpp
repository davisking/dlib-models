...
/*
	...
	Finally, this pretrained model can be freely used by the entire Dlib community
	and has been prepared by Cydral.
*/
...
using agender_type = loss_multiclass_log<fc<2, dropout<relu<fc<16, alevel1<alevel2< input_rgb_image_sized<32>>>>>>>>;
...
int main(int argc, char** argv) try {
		...
		for (auto& img : female_images)
		{
			...
			for (auto face : detector(in))
			{
				...
				if (shape.num_parts())
				{
					...
					if (net(face_chips[0]) == female_label)
						++num_right;
					else
						++num_wrong;					
				}
		...
		for (auto& img : male_images)
		{
			...
			for (auto face : detector(in))
			{
				...
				if (shape.num_parts())
				{
					...
					shapes.push_back(shape);
					extract_image_chips(in, get_face_chip_details(shapes, 32), face_chips);
					if (net(face_chips[0]) == male_label)
						++num_right;
					else
						++num_wrong;
				}
	...
	if (parser.option("test-image"))
	{
		...
		enum label_ gender = female_label;
		for (auto face : detector(in))
		{
			auto shape = sp(in, face);
			if (shape.num_parts())
